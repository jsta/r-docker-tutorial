...
This should lead to you being greeted by the RStudio welcome screen. We will log in using username and password. The username is defined by the rocker/verse image, in this case the user is "rstudio" (see the README.md file of the rocker/verse image for more details  [link](https://github.com/rocker-org/rocker-versioned/blob/master/rstudio/README.md)). The password can be specified by passing `-e PASSWORD=yourpasswordhere` to the docker run command:

~~~
docker run --rm -p 8787:8787 -e PASSWORD=yourpasswordhere rocker/verse
~~~

Or if we don't specify the password, docker will generate one for us and display it in the terminal. For example: `The password is set to phei3sheidoa5Kij`
